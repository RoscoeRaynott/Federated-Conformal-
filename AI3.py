...
if st.session_state.get('analysis_run_complete', False):
    ...
    if additional_viz_choice == "Mean Cluster Curves":
        ...
        # Ensure gene_names_to_use is available from session state
        if not st.session_state.get('gene_names'): # More robust check from session state
            st.warning("Gene names not available for parallel coordinates plot (not in session state).")
            st.stop() # Use st.stop() to halt execution

        gene_names_to_use_viz = st.session_state.gene_names # Use a local variable for clarity

        if not gene_names_to_use_viz: # Check if it's empty after retrieval
            st.warning("No gene names available for parallel coordinates plot.")
            st.stop() # Use st.stop() to halt execution

        num_genes_for_parallel = min(10, len(gene_names_to_use_viz))
        genes_for_plot_parallel = gene_names_to_use_viz[:num_genes_for_parallel]

        if not genes_for_plot_parallel:
            st.warning("No genes selected or available for parallel coordinates plot (after slicing).")
            st.stop() # Use st.stop() to halt execution

        # Ensure combined_data_df_to_use is also checked/retrieved from session state before this block
        if not combined_data_df_to_use.empty and 'Cluster' in combined_data_df_to_use:
            try:
                df_for_parallel = combined_data_df_to_use.copy()
                # ... (rest of your parallel coordinates logic remains the same) ...
                ...
                for gene in genes_for_plot_parallel:
                    if gene in df_for_parallel.columns: # Check if gene column exists
                        df_for_parallel[gene] = pd.to_numeric(df_for_parallel[gene], errors='coerce')
                    else:
                        st.warning(f"Gene column '{gene}' not found in DataFrame for parallel plot. Skipping.")
                        genes_for_plot_parallel.remove(gene) # Remove if not found to avoid later errors

                df_for_parallel = df_for_parallel.dropna(subset=[gene for gene in genes_for_plot_parallel if gene in df_for_parallel.columns])
                ...
                numeric_genes = []
                for gene in genes_for_plot_parallel:
                    if gene in df_for_parallel.columns and pd.api.types.is_numeric_dtype(df_for_parallel[gene]):
                        numeric_genes.append(gene)
                    else:
                        st.warning(f"Gene {gene} is not numeric or was removed, excluding from plot.")
            ...
            st.warning("Clustered data not available for parallel coordinates plot.")
