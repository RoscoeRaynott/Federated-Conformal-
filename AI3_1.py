...
def perform_data_generation(n_c, n_s, n_f):
    st.session_state.data_centers, st.session_state.gene_names = simulate_federated_data_cached(n_c, n_s, n_f)
    st.session_state.data_generated = True
    st.session_state.sim_params = {'n_centers': n_c, 'n_samples': n_s, 'n_features': n_f}
    st.session_state.analysis_run_complete = False

def plot_pca_altair(X_data, labels, title, high_confidence_mask=None):
    try:
        pca_model = PCA(n_components=2)
        ...
        # Tooltip and encoded fields
        tooltips = ["PC1", "PC2", "Cluster"]
        opacity_cond = alt.value(0.9) # Default opacity
        
        if high_confidence_mask is not None:
            plot_df["HighConfidence"] = high_confidence_mask
            tooltips.append("HighConfidence")
            opacity_cond = alt.condition(alt.datum.HighConfidence, alt.value(0.9), alt.value(0.3))
        ...
        chart = (
            ...
            .properties(title=title, width=350, height=300)
            .interactive()
        )
        st.altair_chart(chart, use_container_width=True)
    except Exception as e:
        st.error(f"Error during PCA and plotting for {title}: {e}")
...
if st.sidebar.button("Generate/Update Data", key="generate_data_btn"):
    perform_data_generation(n_centers_input, n_samples_input, n_features_input)
    st.sidebar.success("Data generated/updated!")
...
if st.session_state.get('analysis_run_complete', False) and st.session_state.get('centralized_analysis_complete', False):
    ...
    with col1:
        ...
        # PCA Plot
        plot_pca_altair(X_full, combined_data_df["Cluster_Federated"], "PCA of Federated Clustered Data", combined_data_df["HighConfidence_Federated"])
    ...
    with col2:
        ...
        # PCA Plot
        plot_pca_altair(X_full, combined_data_df["Cluster_Centralized"], "PCA of Centralized Clustered Data")
